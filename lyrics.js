...
function displayLyrics() {
    ...
    lyricsContainer.innerHTML = lyrics.map(lyric => `<div class="lyric" data-time="${lyric.time}">${lyric.text}</div>`).join('');
}
...
function updateLyrics() {
    ...
    lyricsElements.forEach((element, index) => {
        ...
        const nextTime = index < lyrics.length - 1 ? parseFloat(lyricsElements[index + 1].getAttribute('data-time')) : Infinity;
